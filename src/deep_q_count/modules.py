...
from src.deep_q_count.helpers import play_games
...
class Trainer:
    ...
    def train_epoch(self, replay_buffer, batch_size, gamma):
        ...
        moves_t = torch.tensor([self.online_net.moves.index(replay_buffer[i][2]) for i in transition_inds], dtype=torch.int64).unsqueeze(-1)
    ...
    async def eval(self, n_games: int, n_rounds: int, wagers: List[float], game_hyperparams: object):
    ...
    def get_best_state(self):
        return deepcopy(self.online_net.state_dict())