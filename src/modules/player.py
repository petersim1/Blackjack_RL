...
class Player:
    ...
    It is intended to be wrapped into the Game module, which controls the
    blackjack game entirely.

    In the Game module, in each round, this module is reset.

    We don't care for insurance bets, as they are side bets essentially, so
    we'll ignore completely.
    """
    ...
    def _split(self, cards: List[Card]) -> None:
        ...
        i_hand = self.i_hand
        self.aces_split = (self.cards[i_hand].cards[0].card == "A") & (
            self.cards[i_hand].cards[1].card == "A"
        )
        ...
        # Calling this add_cards() fct will inherently create an updated
        # "total" variable.
        # if we create the new Cards in the .insert() method, we won't have
        # the "total" variable yet.
    ...
    def get_value(self) -> Tuple[int, bool]:
        """
        gets the card total and whether there's a useable ace for the
        current hand of cards for a player
        """
        return (
            self.cards[self.i_hand].total,
            self.cards[self.i_hand].useable_ace,
        )
    ...
    def get_valid_moves(self) -> List[str]:
        possible_moves = []
        i_hand = self.i_hand
        if i_hand < 0:  # catches when the player is done. Don't need to additional
            # check .is_done()
        ...
        can_split = (n == 2) and (
            (self.cards[i_hand].cards[0].card == self.cards[i_hand].cards[1].card)
            or (
                (self.cards[i_hand].cards[0].value == self.cards[i_hand].cards[1].value)
                and self.rules.split_any_ten
            )
        )
        can_double = (
            (n == 2)
            and (((n_hands > 1) and self.rules.double_after_split) or (n_hands == 1))
            and can_hit
        )
    ...
    def step(self, move: str, cards_give: List[Card] = []) -> None:
        ...
        assert len(cards_give) == self.get_num_cards_draw(
            move
        ), "Must provide proper # of cards!"
    ...
    def force_completion(self):
        """
        beneficial when the house draws a blackjack, can force the player hand
        to end without needing outside logical checks.
        """
    ...
    def get_result(self, house_cards: Cards) -> Tuple[List[str], float]:
        ...
        for i, hand in enumerate(self.cards):
            val = hand.total
            # 21 after a split is not natural blackjack. It's just 21, even on first two cards. # noqa: E501
