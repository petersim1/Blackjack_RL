...
def gather_buffer_obs(
    replay_buffer: deque[ReplayBuffer], batch_size: int, moves: List[str]
):
    ...
    obs_t = torch.tensor(
        [replay_buffer[i].obs for i in transition_inds], dtype=torch.float32
    )
    action_space = [replay_buffer[i].action_space or ["hit"] for i in transition_inds]
    moves_t = torch.tensor(
        [moves.index(replay_buffer[i].move) for i in transition_inds],
        dtype=torch.int64,
    ).unsqueeze(-1)
    rewards_t = torch.tensor(
        [replay_buffer[i].reward for i in transition_inds], dtype=torch.float32
    ).unsqueeze(-1)
    dones_t = torch.tensor(
        [replay_buffer[i].done for i in transition_inds], dtype=torch.float32
    ).unsqueeze(-1)
    obs_next_t = torch.tensor(
        [replay_buffer[i].obs_next or filler for i in transition_inds],
        dtype=torch.float32,
    )
    action_space_next = [
        replay_buffer[i].action_space_next or ["hit"] for i in transition_inds
    ]
